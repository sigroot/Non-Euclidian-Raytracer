    ...
    for (var j = 0; j < ySize; j++) {
        for (var i = 0; i < xSize; i++) {
            ...
            if (i == 0 || i == xSize-1 || j == 0 || j == ySize-1 || k == 0 || k == zSize-1) {
                ...
                distX = (-10*Math.abs(i-xSize/2)+10*xSize/2+1);
            };
            if (i == 0 || i == xSize-1 || j == 0 || j == ySize-1 || k == 0 || k == zSize-1) {
                distY = 1;
            } else {
                distY = (-10*Math.abs(i-ySize/2)+10*ySize/2+1);
            };
            if (i == 0 || i == xSize-1 || j == 0 || j == ySize-1 || k == 0 || k == zSize-1) {
                distZ = 1;
            } else {
                distZ = (-10*Math.abs(k-zSize/2)+10*zSize/2+1);
            };
...
try {
    fs.writeFileSync(`basicGrid-${String(xSize)}-${String(ySize)}-${String(zSize)}_distortedSpaceExtreme.json`, JSON.stringify(grid));
} catch (err) {
    console.error(err);
}